...
class ICM20948:
    ...
    def _setup(self, bus:I2C_Bus|None=None) -> None:
        '''
    ...
    def _setup_ICM20948(self) -> bool:
        ...
        if self.ICM20948_initialized:
            return self.ICM20948_initialized
    ...
    def InitDMP(self) -> None:
        ...
        # Reset DMP memory
        self._acc.WriteReg(USER_CTRL, 0x08)
        time.sleep(0.1)

        from .icm20948_img_dmp3a_00 import dmp_img
        mem_bank = 0
        ...
        del dmp_img
        from .icm20948_img_dmp3a_26 import dmp_img
        data_pos = 0
        # Write DMP firmware to memory
        while data_pos < len(dmp_img):
            write_len = min(256, len(dmp_img[data_pos:]))
            self.WriteMems(mem_bank, 0, dmp_img[data_pos:data_pos + write_len])
    ...
    def ValidateDMP(self) -> bool:
        '''
        Validate the DMP firmware
        '''

        from .icm20948_img_dmp3a_00 import dmp_img  # So that we know how much to read
        mem_bank = 0
        start_address = DMP_LOAD_START
        data_pos = 0
        check_pos = 0
        ...
        while data_pos < len(dmp_img):
            ...
            read_img = self.ReadMems(mem_bank, start_address, read_len)
            for d in read_img:
                if d != dmp_img[check_pos]:
                    print("00 bank:{} start: {} len: {} checkpos: {}, d: {:02X}, d_ref: {:02X}\n".format(mem_bank, start_address, read_len, check_pos, d, dmp_img[check_pos]))
                    return False
                check_pos += 1
        ...
        del dmp_img
        from .icm20948_img_dmp3a_26 import dmp_img  # So that we know how much to read
        data_pos = 0
        check_pos = 0
        ...
        while data_pos < len(dmp_img):
            ...
            for d in read_img:
                if d != dmp_img[check_pos]:
                    print("26 bank:{} start: {} len: {} checkpos: {}, d: {:02X}, d_ref: {:02X}\n".format(mem_bank, start_address, read_len, check_pos, d, dmp_img[check_pos]))
                    return False
                check_pos += 1
    ...
    def WriteMems(self, bank: int, address: int, data: list) -> None:
        ...
        self.SelectBank(0)
        self._acc.WriteReg(MEM_BANK_SEL, bank)
        self._acc.WriteReg(MEM_START_ADDR, address)
        self._acc.WriteRegs(MEM_R_W, data)
    ...
    def ReadMems(self, bank: int, address: int, len: int):
        ...
        self._acc.WriteReg(MEM_BANK_SEL, bank)
        self._acc.WriteReg(MEM_START_ADDR, address)
        return self._acc.ReadRegs(MEM_R_W, len)
